 ...
 * the License.
 */
package com.gw.springboot.mybatisplus.generatecode.engine;
...
import com.gw.springboot.mybatisplus.generatecode.config.FileOutConfig;
...
import com.gw.springboot.mybatisplus.generatecode.config.rules.FileType;
import com.gw.springboot.mybatisplus.generatecode.extension.EnumClassGenerateUtil;
    ...
    public AbstractTemplateEngine batchOutput() {
        try {
            ...
            for (TableInfo tableInfo : tableInfoList) {
                ...
                // MpEnums.java
                if (null != tableInfo.getEnumsName()
                        && null != pathInfo.get(ConstVal.ENUMS_PATH)
                        && getConfigBuilder().getGlobalConfig().isEnableEnum()) {

                    EnumClassGenerateUtil.generateEnumsFile(tableInfo, this);

//                    if (isCreate(FileType.ENUMS, enumsFiles)) {
//                        writerFile(objectMap, templateFilePath(template.getEnums()), enumsFiles);
//                    }
                }
                ...
                // MpDetailRequest.java
                if (null != tableInfo.getDtoDetailRequestName()
                        && null != pathInfo.get(ConstVal.DTO_REQUEST_PATH)) {
                    String dtoDetailRequestFile =
                            String.format(
                                    (pathInfo.get(ConstVal.DTO_REQUEST_PATH)
                                            + File.separator
                                            + tableInfo.getDtoDetailRequestName()
                                            + suffixJavaOrKt()),
                                    entityName);
                    if (isCreate(FileType.DTO_DETAIL_REQUEST, dtoDetailRequestFile)) {
                        writerFile(objectMap, templateFilePath(template.getDtoDetailRequest()), dtoDetailRequestFile);
                    }
                }

                // MpDetailResponse.java
                if (null != tableInfo.getDtoDetailResponseName()
                        && null != pathInfo.get(ConstVal.DTO_RESPONSE_PATH)) {
                    String dtoDetailResponseFile =
                            String.format(
                                    (pathInfo.get(ConstVal.DTO_RESPONSE_PATH)
                                            + File.separator
                                            + tableInfo.getDtoDetailResponseName()
                                            + suffixJavaOrKt()),
                                    entityName);
                    if (isCreate(FileType.DTO_DETAIL_RESPONSE, dtoDetailResponseFile)) {
                        writerFile(objectMap, templateFilePath(template.getDtoDetailResponse()), dtoDetailResponseFile);
                    }
                }

                // MpDeleteRequest.java
                if (null != tableInfo.getDtoDeleteRequestName()
                        && null != pathInfo.get(ConstVal.DTO_REQUEST_PATH)) {
                    String dtoDeleteRequestFile =
                            String.format(
                                    (pathInfo.get(ConstVal.DTO_REQUEST_PATH)
                                            + File.separator
                                            + tableInfo.getDtoDeleteRequestName()
                                            + suffixJavaOrKt()),
                                    entityName);
                    if (isCreate(FileType.DTO_DELETE_REQUEST, dtoDeleteRequestFile)) {
                        writerFile(objectMap, templateFilePath(template.getDtoDeleteRequest()), dtoDeleteRequestFile);
                    }
                }
                ...
                if (null != tableInfo.getFeignClientConstName()
                    ...
                    if (isCreate(FileType.FEIGN_CLIENT_CONST, feignClientConstFiles)) {
                        writerFile(objectMap, templateFilePath(template.getFeignClientConst()), feignClientConstFiles);
                    }
                }
    ...
    public void writerFile(Map<String, Object> objectMap, String templatePath, String outputFile)
            throws Exception {
